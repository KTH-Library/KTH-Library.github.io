---
title: 'Workshop RSO / KTHB'
author: ""
date: '2021-05-27'
output:
  ioslides_presentation:
    css: assets/css/styles.css
    logo: assets/img/logo.svg
    mathjax: default
    self-contained: yes
    transition: slower
    widescreen: yes
  beamer_presentation: default
---
  
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

library(ktheme)
library(webshot)

red <- function(txt) 
  sprintf("<font color='%s'>\n%s\n</font>", tolower(palette_kth()["cerise"]), txt)

#webshot::install_phantomjs()
#unlink("retrosearch-screenshot.png")

suppressPackageStartupMessages(library(magick))

# BG images that could be used

```

## Agenda

- ÅBU-appen
- Dataflöden
- Gruppdiskussion i smågrupper
  - Vilka datakällor/dataflöden använder du som vore intressanta att samköra med det som visats?
  - Vilka manuella analyser och rapporter producerar du som vore intressanta att automatisera?
- Summering

<!--
https://intra.kth.se/itm/nyheter/mikael-ostling-rankingens-krigare-1.1075812 
-->

## Bakgrund & bibliometrisk användning

Exempel på användning av bibiometriska data

- Resursfördelning (nationellt och inom KTH)
- Ranking
- Uppföljning inom KTH
- RAE
- Projektansökningar

**Leveranser**:  

- Rapportunderlag
- ÅBU (app)
- Ad-hoc analys

# Demo av ÅBU-appen

- Årlig bibliometrisk uppföljning: [ÅBU/ABM](https://kth.se/abm)
- Personalbaserad ÅBU: [authorbased ABM](https://bibliometrics.lib.kth.se/beta/authorbased)

## Dataflöden

- Datakällor  
  Strukturerade data inom och utanför KTH

- Dataflöden   
  Hur flyttar/reser data inom organisationen eller för olika behov?
  
- Entiteter
  Används som noder vid analys eller för att knyta ihop data
  - Forskare
  - Organisationer på KTH
  - Publikationer
  - Indikatorer
  
- Analysbehov  
 "Slutanvändare" och beroende av data och fungerande dataflöden 

## Exempel på dataflöde (data om forskare från HR)

![HR-data i Minio](buckets.png){height=500px}

## Dataflöden i ÅBU-projektet

![Dataflöden](dataflow.png){height=500px}

## Utblick - dataflöden & tjänster

- Vad är poängen med dataflöden?  <img align="right" height="200" src="minio.png">
    - transparens & tillgänglighet
    - reproducerbarhet
    - analytics
    - standardisering
- Infrastruktur   <img align="right" height="300" src="https://cdn.hackernoon.com/hn-images/1*7IMev5xslc9FLxr9hHhpFw.png">
  - Datatillgänglighet
  - Object storage (S3)
  - Knyta samman datakällor

<!-- ![Källa: https://hackernoon.com/the-ai-hierarchy-of-needs-18f111fcc007](https://cdn.hackernoon.com/hn-images/1*7IMev5xslc9FLxr9hHhpFw.png){height=200px} -->

## Gruppdiskussion

  - Kan vi komplettera vår app eller andra appar med era datakällor?
  - Vilka manuella analyser kan automatiseras på liknande sätt?
  - Vilka data har ni att lägga in i Minio för tester (som kopplar KTHs grundenheter; kthid, org-sluggar etc)?

##


#

##

![Analytical data flow](overview.svg){width=800;}

##

![Källa: KTHB/PIMS](data-overview-kthb.png){height=500px}

## Exempel på dataflöde

- Lägg upp data
- Använd i en app




  
## Automatiserade analyser

## Summering

Presentation av gruppdiskussion
