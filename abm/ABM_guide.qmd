...
compiled in October - November each year and is presented for the
last 10 full years.
...
[OpenAlex](https://openalex.org). For a publication to be included, it
needs to be marked as a peer reviewed publication in DiVA and have a
Digital Object Identifier (DOI) that OpenAlex can recognice and
determine an OA status for.

The statuses are presented as defined by OpenAlex with the exeption that we
count bronze publications as closed. For further information, see
[OpenAlex Support pages](https://help.openalex.org/hc/en-us/articles/24347035046295-Open-Access-OA).

Note that there is also a separate report only dealing with [Open Access at KTH](https://bibliometrics.lib.kth.se/OA-rapport.html). In addition to Open access-status across time
that report also presents OA status for thesis, Article processing charges (APC) and OA-status for individual 
